...
for i in range(qtd_medidas):
    ...
    for x in notas:
        ...
        f = interp1d(int_x, reduce(lambda a,b: a + b, [n.array([gauss(v, each, std) for v in int_x]) for each in notas])/7)
...
#p.hist([r.choice(notas_todas[i]) for i in range(1)], normed=1)
p.show()
