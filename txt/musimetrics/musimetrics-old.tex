...
on
philosophy~\cite{Fabbri}. The opposition, strong on
...
Opposite state & $\vec{r}_i = \vec{v}_i + 2(\vec{a}_i - \vec{v}_i)  = 2 \vec{a}_i - \vec{v}_i$  \\ \\
...
Counter-dialectics \\ index & $d_{i \rightarrow k} = \frac{\left< \vec{v}_j - \vec{v}_i, \vec{v}_k \right> + \frac{\vec{v}_i^2 - \vec{v}_j^2}{2}}{\sqrt{(\vec{v}_j-\vec{v}_i)^2}}$ \\ \\ 

%Counter-dialectics \\index & $\rho_{i \rightarrow k} = \frac{d_{i \rightarrow k}}{|| \vec{M}_{i,j}||}$ \\ \\
...
instance, the Pearson correlation coefficient of 0.65 was obtained for
